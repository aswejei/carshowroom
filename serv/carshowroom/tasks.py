...
from core.operations import parse_dict_to_Q_expr, buy_car_from_supplier
...
def buy_cars():
    for showroom in CarShowroom.objects.filter(is_active=True):
        ...
        car_price_query = CarPriceRelationSupplier.objects.filter(is_active=True).filter(q_expr).order_by(
            'price_with_discount')
...
def generate_offer(max_price=None, characteristics_dict=None, request=None):
    ...
    car_price = CarPriceRelationShowroom.objects.filter(is_active=True).filter(Q(
        **{'price_with_discount__lte': max_price})).filter(q_expr).order_by('price_with_discount').first()
    if car_price:
        offer = CustomerShowroomOffers.objects.create(car=car_price.car, customer=request.user.customer_profile,
                                                      showroom=car_price.showroom, price=car_price.price_with_discount)
        return offer
    else:
        return None
