...
from carshowroom.models import CarPriceRelationShowroom, CarShowroom, CustomerShowroomOffers
from customer.models import Customer
from supplier.models import CarPriceRelationSupplier, ShowroomSupplierOffers
...
def buy_car_from_supplier(car_price_supplier: CarPriceRelationSupplier, showroom: CarShowroom, budget=None):
    ...
    try:
        car_price_showroom = CarPriceRelationShowroom.objects.filter(showroom__id=showroom.id).select_related(
            'car').get(
            car__id=car_price_supplier.car.id)
    ...
    if car_price_showroom:
        ...
        car_price_showroom = CarPriceRelationShowroom.objects.create(car=car_price_supplier.car, showroom=showroom,
                                                                     price=car_price_supplier.price * 1.5, quantity=1)
        car_price_showroom.save()
    ShowroomSupplierOffers.objects.create(car=car_price_supplier.car, supplier=car_price_supplier.supplier,
                                          showroom=showroom, price=car_price_supplier.price_with_discount)
    return budget


def buy_car_from_showroom(car_price_showroom: CarPriceRelationShowroom, customer: Customer):
    if car_price_showroom:
        CustomerShowroomOffers.objects.create(customer=customer, showroom=car_price_showroom.showroom,
